...
const TextField = ({ formik }) => {
    ...
    // click handler
    const addInputValue = () => {
            dispatch(addInputBox({ term: formik.values.term, defination: formik.values.defination }));
            formik.values.term = '';
        ...
        <div>
            <div id='form_resp' className='flex flex-col sm:justify-start sm:flex-row'>
                <div className='relative flex flex-col sm mr-4 mb-3 sm:mb-0 sm:mt-4 w-full sm:w-2/6'>
                    ...
                    {formik.errors.term ? <div className='text-red-600 text-sm'>{formik.errors.term}</div> : null}
                </div>
                <div className='flex flex-col w-full sm:mt-4 sm:w-2/6'>
                    ...
                    {formik.errors.defination ? <span className='text-red-600 text-sm'>{formik.errors.defination}</span> : null}
                </div>
            </div>
            <span
